		...
		if (count($thumbs) > $mincount) { 
			...
			<?php foreach ($thumbs as $tt) :  global $thumb; $thumb = $tt; ?>
				...
				<div id="thumbnail_frame">
					<a href="<?php wppa_photo_page_url(); echo($occ); ?>" class="img" id="a<?php echo($thumb['id']); ?>">
						<img src="<?php echo($src); ?>" alt="<?php echo($thumb['name']); ?>" title="<?php echo($thumb['name']); ?>" style="<?php echo($imgattr); ?>" <?php echo($events) ?>/>
					</a>
				</div>
			<?php endforeach; ?>
			<div id="wppa-popup" onmouseout="this.innerHTML=''"></div>
			<div id="wppa-debug" class="clear"></div>
			</div>
...
?>
	<div id="prev-arrow" class="prev arrow"><a id="p-a" onclick="wppa_prev()">&laquo;</a></div><div id="next-arrow" class="next arrow"><a id="n-a" onclick="wppa_next()">&raquo;</a></div>
