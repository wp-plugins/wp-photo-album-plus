...
function wppaConsoleLog(arg,force=false){if(typeof(console)!='undefined'&&(wppaDebug||force)){console.log(arg);}}
...
function wppaAjaxUploadFront(mocc){var xmlhttp=wppaGetXmlHttp();var alb=jQuery('#wppa-upload-'+mocc).attr('value');var file=jQuery('#wppa-user-upload-'+alb+'-'+mocc).attr('value');var nonce=jQuery('#wppa-nonce').attr('value');var data='action=wppa&wppa-action=do-fe-upload'+'&wppa-nonce='+nonce+'&wppa-upload-album='+alb+'&wppa-user-upload-'+alb+'-'+mocc+'[]=file';xmlhttp.open('POST',wppaAjaxUrl,false);xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");xmlhttp.send(data);if(xmlhttp.readyState==4&&xmlhttp.status==200){var result=xmlhttp.responseText;alert(result);}
else{alert('Comm error '+xmlhttp.status+' encountered');return false;}
return false;}