...
var photoId=wppaUrlToId(wppaOvlUrl);_bumpViewCount(photoId);if(wppaOvlMode!='normal'){jQuery('#wppa-overlay-bg').fadeTo(300,wppaOvlOpacity);var html='<div id="wppa-ovl-full-bg" style="position:fixed; width:'+wppaWindowInnerWidth+'px; height:'+wppaWindowInnerHeight+'px; left:0px; top:0px; text-align:center;" >'+'<img id="wppa-overlay-img"'+' ontouchstart="wppaTouchStart( event, \'wppa-overlay-img\', -1 );"'+' ontouchend="wppaTouchEnd( event );"'+' ontouchmove="wppaTouchMove( event );"'+' ontouchcancel="wppaTouchCancel( event );"'+' src="'+wppaOvlUrl+'"'+' style="border:none; width:'+wppaWindowInnerWidth+'px; visibility:hidden; box-shadow:none; position:absolute;"'+' />'+' <div style="height: 20px; width: 100%; position:absolute; bottom:0; left:0;" onmouseover="jQuery(\'#wppa-ovl-legenda-2\').css(\'visibility\',\'visible\');" onmouseout="jQuery(\'#wppa-ovl-legenda-2\').css(\'visibility\',\'hidden\');wppaShowLegenda=\'hidden\';" >'+' <div id="wppa-ovl-legenda-2" style="position:absolute; left:0; bottom:0; background-color:'+(wppaOvlTheme=='black'?'#272727':'#a7a7a7')+'; color:'+(wppaOvlTheme=='black'?'#a7a7a7':'#272727')+'; visibility:'+wppaShowLegenda+';" >'+'Mode='+wppaOvlMode+'. Keys: f = next mode; esc,x,o,c,q = exit; p = previous, n = next, s = start/stop, d = dismiss this notice.'+' </div>'+' </div>';' </div>';jQuery('#wppa-overlay-ic').html(html);setTimeout('wppaOvlShowFull()',10);return false;}
...
function wppaOvlHide(){wppaConsoleLog('wppaOvlHide',1);jQuery('#wppa-overlay-ic').html('');jQuery('#wppa-overlay-ic').css({paddingTop:0});jQuery('#wppa-overlay-bg').fadeOut(300);jQuery(document).off('keydown',wppaOvlKeyboardHandler);wppaOvlFirst=true;wppaOvlRunning=false;wppaOvlMode='normal';jQuery('#wppa-overlay-sp').css({visibility:'hidden'});}
