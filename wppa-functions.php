...
//		 ( wppa_get_get('album', false) !== false || wppa_get_get('photo') ) &&				// ... but not sufficient. Must have at least also album or photo
	...
	if ( $wppa['src'] ) {	// It is occur == 1 on search always
		...
		wppa_dbg_msg( 'Rootsearch='.( $wppa['is_rootsearch'] ? 'yes' : 'no'), 'red' );
		wppa_dbg_msg( 'Root(509)='.(isset($_SESSION['wppa_session']['search_root'])?$_SESSION['wppa_session']['search_root']:''),'red' );
	...
	elseif ( $wppa['occur'] == '1' ) {	// Not search while occur == 1, 
		...
		wppa_dbg_msg( 'Potential search root = '.$wppa['start_album'], 'red' );
		$_SESSION['wppa_session']['search_root'] = $wppa['start_album'];
		// Reset the rootbox on next pageload
		$_SESSION['wppa_session']['rootbox'] = '0';
	}
...
global $thumb;
	...
	if ( ! $name ) $name = '&nbsp;';
	$fullname = wppa_get_photo_name( $id, $wppa_opt['wppa_show_full_owner'], true, true );
