	...
	elseif ( strlen($src) && $wppa['master_occur'] == '1' ) {	// Search is in occur 1 only
		...
		foreach ( $tmbs as $thumb ) {
			if ( ! $wppa_opt['wppa_excl_sep'] || ! wppa_is_separate($thumb['album']) ) {	// Not exclude sepreate or not seperate
				...
				if ( $wppa_opt['wppa_search_tags'] ) {
					$haystack .= ' '.str_replace(',', ' ', $thumb['tags']);
				}
				if ( wppa_deep_stristr($haystack, $src) ) {
					$thumbs[] = $thumb;
				}
			}
		}
...
global $album;
	...
	elseif ($action == 'close') {
		wppa_user_upload_html($wppa['current_album'], wppa_get_container_width('netto'), false);
...
global $wpdb;
	...
	$wppa['out'] .= wppa_nltab('+').'<div id="album-'.$album['id'].'-'.$wppa['master_occur'].'" class="album wppa-box wppa-cover-box wppa-cover-box-'.$wppa['master_occur'].' wppa-'.$wppa_alt.'" style="'.$style.'" >';
		...
		wppa_user_upload_html($album['id'], wppa_get_cover_width('cover'), true);
...
function wppa_user_upload_html($alb, $width, $oncover = true) {
global $wppa;
global $wppa_opt;
...
global $album;
	...
	else wppa_save_source($file['tmp_name'], $name, $alb);
