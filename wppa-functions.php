...
function wppa_get_image_page_url_by_id($id, $single = false) {
global $wppa;
global $thumb;
	...
	if ( $single ) $result = wppa_get_permalink().'wppa-single=1&amp;wppa-album='.$thumb['album'].'&amp;wppa-photo='.$thumb['id'].'&amp;wppa-cover=0&amp;wppa-'.$w.'occur='.$occur;	
	else $result = wppa_get_permalink().'wppa-album='.$thumb['album'].'&amp;wppa-photo='.$thumb['id'].'&amp;wppa-cover=0&amp;wppa-'.$w.'occur='.$occur;	
...
global $wppa_opt;
	...
	elseif ( $wppa['is_tag'] ) {
		...
		foreach ( array_keys($temp) as $index ) {
			if ( in_array($temp[$index]['id'], $tags[$wppa['is_tag']]['ids']) ) {
				$thumbs[] = $temp[$index];
			}
		}
	}
...
//		else
		...
		if ( $wppa['is_lasten'] ) {
			$url .= 'wppa-lasten='.$wppa['lasten_count'].'&';
		}

		if ( $wppa['is_comten'] ) {
			$url .= 'wppa-comten='.$wppa['comten_count'].'&';
		}

		if ( $wppa['is_tag'] ) {
			$url .= 'wppa-tag='.$wppa['is_tag'].'&';
		}
...
//			wppa_dbg_q('Q46');
		...
		if ( $iret !== false ) {
			if ( $status != 'spam' ) {
				if ($cedit) {
					if ( $wppa_opt['wppa_comment_notify_added'] ) echo('<script type="text/javascript">alert("'.__a('Comment edited', 'wppa_theme').'")</script>');
				}
				else {
					...
					// Notyfy user
					if ( $wppa_opt['wppa_comment_notify_added'] ) echo('<script type="text/javascript">alert("'.__a('Comment added', 'wppa_theme').'")</script>');
				}
			}
			else {
...
global $thlinkmsggiven;
	...
	$wppa['out'] .= wppa_nltab('+').'<div id="thumb-'.$thumb['id'].'-'.$wppa['master_occur'].'" class="thumb wppa-box wppa-cover-box wppa-cover-box-'.$wppa['master_occur'].' wppa-'.$wppa_alt.'" style="'.$style.'" >';
		...
		$wppa['out'] .= wppa_nltab('+').'<div id="thumbtext_frame_'.$thumb['id'].'_'.$wppa['master_occur'].'" class="wppa-text-frame-'.$wppa['master_occur'].' wppa-text-frame thumbtext-frame" '.$textframestyle.'>';
			...
			$wppa['out'] .= wppa_nltab('-').'</h2>';
			$desc = $thumb['status'] == 'pending' ? '<span style="color:red">'.__a('Awaiting moderation', 'wppa_theme').'</span>' : wppa_get_photo_desc($thumb['id']);
...
//		$title = esc_attr(__($title));
	...
	//			&& ! $wppa_opt['wppa_use_thumb_popup']									// links from popups now also supported
			...
			{ 	// Ajax	possible
				...
				$onclick = "wppaDoAjaxRender(".$wppa['master_occur'].", '".wppa_get_slideshow_url_ajax($wppa['start_album'], '0').'&amp;wppa-photo='.$thumb['id']."', '".wppa_get_slideshow_url('0')."&amp;wppa-photo=".$thumb['id']."')";

				$wppa['out'] .= wppa_nltab('+').'<a style="position:static;" class="thumb-img" id="x-'.$thumb['id'].'-'.$wppa['master_occur'].'">';
					$wppa['out'] .= wppa_nltab().'<img onclick="'.$onclick.'" id="i-'.$thumb['id'].'-'.$wppa['master_occur'].'" src="'.$url.'" alt="'.$altforpopup.'" title="'.esc_attr($title).'" width="'.$imgwidth.'" height="'.$imgheight.'" style="'.$imgstyle.' cursor:pointer;" '.$events.' />';
				$wppa['out'] .= wppa_nltab('-').'</a>';
...
global $wppa_opt;
	...
	if ( $wppa['is_single'] && is_feed() ) {	// process feed for single image slideshow here, normal slideshow uses filmthumbs
		...
		$wppa['out'] .= wppa_nltab().'<a href="'.get_permalink().'"><img src="'.wppa_get_photo_url($wppa['start_photo']).'" style="'.$style.'" width="'.$width.'" height="'.$height.'" /></a>';
		return;
	}
...
//	elseif ( $wppa_opt['wppa_enable_slideshow'] ) {
	...
	if (is_feed()) {
		...
		if ( $wppa_opt['wppa_film_linktype'] == 'lightbox' && $tmp == 'film' ) {
			$wppa['out'] .= wppa_nltab('+').'<a href="'.$furl.'" rel="'.$wppa_opt['wppa_lightbox_name'].'[occ'.$wppa['master_occur'].']" title="'.wppa_get_lbtitle('slide', $thumb['id']).'" >';
		}	
...
global $wppa_opt;
	...
	$query = $wpdb->prepare('INSERT INTO `' . WPPA_PHOTOS . '` (`id`, `album`, `ext`, `name`, `p_order`, `description`, `mean_rating`, `linkurl`, `linktitle`, `linktarget`, `timestamp`, `owner`, `status`, `tags`) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', $id, $alb, $ext, $name, $porder, $desc, $mrat, $linkurl, $linktitle, $linktarget, time(), $owner, $status, '');
	wppa_dbg_q('Q58');
...
global $wpdb;
	...
	$pos = strpos($temp, '2#');
	while ( $pos !== false ) {
		$tmp = substr($temp, 0, $pos).__a('n.a.', 'wppa_theme').substr($temp, $pos+5);
		$temp = $tmp;
		$pos = strpos($temp, '2#');
	}
...
global $exifdata, $exifdataphoto;
	...
	$pos = strpos($temp, 'E#');
	while ( $pos !== false ) {
		$tmp = substr($temp, 0, $pos).__a('n.a.', 'wppa_theme').substr($temp, $pos+6);
		$temp = $tmp;
		$pos = strpos($temp, 'E#');
	}
...
global $thumb;
	...
	$result = '';
	if ( $do_name ) $result .= wppa_get_photo_name($thumb['id']); //__(stripslashes($thumb['name']));
...
global $thumb;
	...
	$photo_desc = wppa_strip_tags(wppa_html(__(stripslashes($thumb['description']))), 'all');
	
	// The default description
	$see_on_site = sprintf(__a('See this image on %s', 'wppa_theme'), str_replace('&amp;', __a('and', 'wppa_theme'), get_bloginfo('name')));
...
global $wppa;
	...
	if ( $wppa_opt['wppa_tagcloud_linkpage'] ) {
		...
		return __a('Please select a tagcloud landing page', 'wppa_theme');
	}
	$tags = wppa_get_taglist();
	if ( $tags ) {
		$top = '0';
