		...
		if ( substr( $wppa['start_album'], 0, 1 ) == '#' ) {		// Keyword
			...
			switch ( $keyword ) {		//	( substr( $wppa['start_album'], 0, 5 ) ) {	
				...
				case '#owner':
					...
					if ( $parent === false ) return;
					if ( $parent ) {
						$albs = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `".WPPA_ALBUMS."` WHERE `owner` = %s AND `a_parent` = %s", $owner, $parent ), ARRAY_A );
					}
					else {	// Generic plus separate if owner requested
						$albs = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `".WPPA_ALBUMS."` WHERE `owner` = %s AND `a_parent` < '1'", $owner ), ARRAY_A );
					}
					wppa_dbg_q( 'Q-main-5');
					wppa_cache_album( 'add', $albs );
					$id = '';
...
global $wppa_session;
	...
	elseif ( $wppa['is_upldr'] ) {
		$max = '1000000';
		$thumbs = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `".WPPA_PHOTOS."` WHERE `owner` = %s AND ( ( `status` <> 'pending' AND `status` <> 'scheduled' ) OR `owner` = %s ) ORDER BY `timestamp` DESC LIMIT %d", $wppa['is_upldr'], wppa_get_user(), $max ), ARRAY_A );
		wppa_dbg_q( 'Q-UPL' );
	}
	// Topten?	
	elseif ( $wppa['is_topten'] ) {
		...
		if ( $alb ) $thumbs = $wpdb->get_results( "SELECT * FROM `".WPPA_PHOTOS."` WHERE ( `album` = ".$alb." ) ORDER BY ".$sortby." LIMIT ".$max, ARRAY_A );
		else $thumbs = $wpdb->get_results( "SELECT * FROM `".WPPA_PHOTOS."` ORDER BY ".$sortby." LIMIT ".$max, ARRAY_A );
		wppa_dbg_q( 'Q-TT' );
	}
	// Featen?
			...
			if ( ! $no_popup && wppa_switch( 'wppa_use_thumb_popup' ) ) {
				if ( wppa_opt( 'wppa_thumb_linktype' ) != 'lightbox' ) {
					...
					if ( wppa_is_video( $id ) ) {
						...
						$thumbsize = getimagesize( wppa_get_thumb_path( $id ) );
						$x = $thumbsize['0'];
						$y = $thumbsize['1'];
					}
...
static $auto;
	...
	if ( $action == 'open' ) {
		...
		if ( wppa_switch( 'wppa_allow_ajax' ) ) {
			if ( ! $wppa['in_widget'] ) {
				$wppa['out'] .= wppa_nltab( '+' ).'<div class="wppa-container-'.$wppa['mocc'].'" style="text-align:center; width:'.wppa_get_container_width().'px;" ><img id="wppa-ajax-spin-'.$wppa['mocc'].'" src="'.wppa_get_imgdir().'loader.gif" style="box-shadow:none; z-index:1010; position:absolute; margin-top: 200px; margin-left:-32px; display:none;"></div>';
			}
		}
		...
		wppa_add_js_page_data( wppa_nltab().'<script type="text/javascript">' );
			...
			if ( 'ss' == $wppa['in_widget'] || wppa_page( 'slide' ) ) {
	...
	elseif ( $action == 'close' )	{
		...
		if ( $wppa['debug'] ) {
			...
			wppa_dbg_msg( 'Number of queries occ '.$wppa['mocc'].':'.$wppa_numqueries );
			wppa_dbg_q( 'print' );
		}
	}
	else {
...
global $thlinkmsggiven;
	...
	$wppa['out'] .= wppa_nltab( '+' ).'<div id="thumb-'.$id.'-'.$wppa['mocc'].'" class="thumb wppa-box wppa-cover-box wppa-cover-box-'.$wppa['mocc'].' wppa-'.$wppa_alt.'" style="'.$style.'" >';
...
global $wpdb;
	...
	else $link = wppa_get_imglnk_a( 'thumb', $id );
	...
	if ( $link ) {
		if ( $link['is_url'] ) {	// is url
			...
			{ 	// Ajax	possible
				...
				if ( $is_video ) { 
					$wppa['out'] .= wppa_nltab().'<video onclick="'.$onclick.'" id="i-'.$id.'-'.$wppa['mocc'].'" '.$imgalt.' title="'.$title.'" width="'.$imgwidth.'" height="'.$imgheight.'" style="'.$imgstyle.' cursor:pointer;" '.$events.' >'.wppa_get_video_body( $id ).'</video>';
				}
				else {
				...
				if ( $is_video ) { 
					$wppa['out'] .= wppa_nltab().'<video id="i-'.$id.'-'.$wppa['mocc'].'" '.$imgalt.' title="'.$title.'" width="'.$imgwidth.'" height="'.$imgheight.'" style="'.$imgstyle.' cursor:pointer;" '.$events.' >'.wppa_get_video_body( $id ).'</video>';
				}
				else {
		...
		elseif ( $link['is_lightbox'] ) {	// link is lightbox
			...
			if ( $is_video ) { 
				$wppa['out'] .= wppa_nltab().'<video id="i-'.$id.'-'.$wppa['mocc'].'" '.$imgalt.' title="'.wppa_zoom_in().'" width="'.$imgwidth.'" height="'.$imgheight.'" style="'.$imgstyle.$cursor.'" '.$events.' >'.wppa_get_video_body( $id ).'</video>';
			}
			else {
		...
		if ( wppa_switch( 'wppa_use_thumb_popup' ) ) {
			$wppa['out'] .= wppa_nltab( '+' ).'<div id="x-'.$id.'-'.$wppa['mocc'].'">';
				if ( $is_video ) { 
					$wppa['out'] .= wppa_nltab().'<video '.$imgalt.' title="'.$title.'" width="'.$imgwidth.'" height="'.$imgheight.'" style="'.$imgstyle.'" '.$events.' >'.wppa_get_video_body( $id ).'</video>';
				}
				else {
	...
	if ( ! $com_alt ) {	
		...
		if ( wppa_switch( 'wppa_thumb_text_name' ) ) {
			$wppa['out'] .= wppa_nltab().'<div class="wppa-thumb-text" style="'.__wcs( 'wppa-thumb-text' ).'" >';
				if ( wppa_switch( 'wppa_thumb_text_name' ) ) $wppa['out'] .= wppa_get_photo_name( $id, wppa_switch( 'wppa_thumb_text_owner' ) ); // wppa_qtrans( stripslashes( $thumb['name'] ) );
			$wppa['out'] .= '</div>';
		}
		// Share
		...
		if ( wppa_opt( 'wppa_film_linktype' ) == 'lightbox' && $tmp == 'film' ) {
			...
			if ( $tmp == 'film' && ! $com_alt && ! wppa_cdn() ) $wppa['out'] .= '<!--';
				if ( wppa_is_video( $thumb['id'] ) ) {
					...
					$wppa['out'] .= '<img id="wppa-'.$tmp.'-'.$idx.'-'.$wppa['mocc'].'" class="wppa-'.$tmp.'-'.$wppa['mocc'].'" src="'.$url.'" alt="'.$alt.'" style="'.$imgstyle.$cursor.'" '.$events.' />';
				}
			if ( $tmp == 'film' && ! $com_alt && ! wppa_cdn() ) $wppa['out'].='-->';
...
global $wppa;
	...
	$wppa['out'] .='" style="width: '.$captwidth.'px">';
		...
		if ( wppa_is_video( $wppa['single_photo'] ) ) {
			...
			$wppa['out'] .= wppa_nltab().'<img src="'.$src.'" alt="" style="'.$style.'" class="size-medium wppa-mphoto wppa-mimg-'.$wppa['mocc'].'" title="'.$title.'" width="'.$width.'" height="'.$height.'" />';
		}
		if ( $link ) {
			$wppa['out'] .= wppa_nltab( '-' ).'</a>';
		}
...
global $wppa;
	...
	if ( $do_download ) {
		if ( wppa_opt( 'wppa_art_monkey_display' ) == 'button' ) {
			$result .= '<input type="button" title="Download" style="cursor:pointer; margin-bottom:0px; max-width:500px;" class="wppa-download-button" onclick="wppaAjaxMakeOrigName( '.$wppa['mocc'].', '.$id.' );" value="'.__a('Download:').' '.wppa_get_photo_name( $id ).'" />';
		}
		else {
			$result .= '<a title="Download" style="cursor:pointer;" onclick="wppaAjaxMakeOrigName( '.$wppa['mocc'].', '.$id.' );" >'.__a('Download:').' '.wppa_get_photo_name( $id ).'</a>';
		}
	}
	else {
