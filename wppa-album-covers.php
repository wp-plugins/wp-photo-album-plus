...
global $wppa;
	...
	if ( '0' == $id ) {
		if ( current_user_can( 'wppa_moderate' ) ) {
			$temp = $wpdb->get_results( $wpdb->prepare( 
				"SELECT * FROM `" . WPPA_PHOTOS . "` WHERE `album` = %s ORDER BY RAND( " . $wppa['randseed'] . " ) LIMIT %d", 
				$alb, $count ), ARRAY_A );
		}
		else {
			$temp = $wpdb->get_results( $wpdb->prepare( 
				"SELECT * FROM `" . WPPA_PHOTOS . "` WHERE `album` = %s AND ( ( `status` <> 'pending' AND `status` <> 'scheduled' ) OR `owner` = %s ) ORDER BY RAND( " . $wppa['randseed'] . " ) LIMIT %d", 
	...
	if ( '-1' == $id ) {		
		$temp = $wpdb->get_results( $wpdb->prepare( 
			"SELECT * FROM `" . WPPA_PHOTOS . 
			"` WHERE `album` = %s AND `status` = 'featured' ORDER BY RAND( " . $wppa['randseed'] . " ) LIMIT %d", 
			$alb, $count ), ARRAY_A );
	}
